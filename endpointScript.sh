...
curl -i -s https://api-spec-proxy.herokuapp.com/proxies/1/proxy/employees | head -n 1|cut -d$' ' -f2
sleep 1
echo "GET non existing endpoint to the proxy"
curl -i -s https://api-spec-proxy.herokuapp.com/proxies/1/proxy/nonExistingEndpoint | head -n 1|cut -d$' ' -f2
sleep 1
